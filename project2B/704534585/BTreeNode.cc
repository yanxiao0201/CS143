                              ...
                              BTLeafNode& sibling, int& siblingKey)
{
    ...
    //? still need to change the pageid for the old page. Remember this!! May need to change after setting up other parts
...
int BTNonLeafNode::getKeyCount()
{
    ...
    int maxnum = limit/length; //first key being empty
...
RC BTNonLeafNode::locateChildPtr(int searchKey, PageId& pid)
{
    ...
    char * bufpointer = buffer;
    for (i = 0; i < keycount * length + length; i+=length){
        memcpy(&key, bufpointer, sizeof(int));
        if (key <= searchKey){
            bufpointer += length;
        }
...
RC BTNonLeafNode::initializeRoot(PageId pid1, int key, PageId pid2)
{
    ...
    memcpy(buffer + length, &key, sizeof(int));
    memcpy(buffer + length + sizeof(int), &pid2, sizeof(PageId));
